...
- `reversed <http://docs.python.org/library/functions.html#reversed>`_
   ...
   - **range** returns a list, **xrange** and **reversed** returns an
       iterator.
...
- Items in **Generator** are evaluated on demand.

- http://stackoverflow.com/a/102632
...
.. code-block:: python
   ...
   new_data = []
   for data in very_large_data:
       data2 = pre_process1(data)
       data3 = post_process1(data2)
       new_data.append(data3)

You can make this into a two-step process using a temporary list.
...
.. code-block:: python
   ...
   def post_process(data_list):
       new_data_list = []
       for data in data_list:
           data2 = post_process1(data)
           new_data.append(data2)

   new_data = post_process(pre_process(data_list))
...
.. code-block:: python
   ...
   def pre_process(data_list):
       for data in data_list:
           yield pre_process1(data)

   def post_process(data_list):
       for data in data_list:
           yield post_process1(data)

   new_data = list(post_process(pre_process(data_list)))

- Note that excution sequnce of **pre_process1** and **post_process1**
  is same as our original example.
...
.. code-block:: python
    ...
    m2 = p.match("06Mar1232.fits") # m2.group() => '06Mar1232.fit'
    m3 = p.match("2012_06Mar1232.fits") # None

    m4 = p.search("2012_06Mar1232.fits") # m4.group() => '06Mar1232.fit'
