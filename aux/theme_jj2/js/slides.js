  ...
  Slide.prototype = {
      ...
      if (state == "current" && !this._visited) {
        ...
        if (!disableCodeHighlights) {
          ...
          for (var i = 0, pre; pre = pres[i]; ++i) {
            util.removeClass(pre, 'codehighlight');
          }
        }
      }
    ...
    _makeBuildList: function() {
      ...
      if (this._node) {
        ...
        this._buildList = util.query(".build > *", this._node);
      }
    ...
    _runAutos: function() {
      ...
      if (idx >= 0) {
        ...
        if (canTransition()) {
          ...
          util.addClass(elem, "active-build");
        } else {
          setTimeout(function() {
            util.removeClass(elem, "to-build");
            util.addClass(elem, "active-build");
    ...
    buildNext: function() {
      ...
      elem = this._buildList.shift()
      util.removeClass(elem, "to-build");

      active_list = util.query(".active-build", this._node);
      active_list.forEach(function(el) {
        util.removeClass(el, "active-build");
      });
  ...
  SlideShow.prototype = {
    ...
    highlightImportantCode: function() {
      ...
      util.query('.current pre').forEach(function(pre) {
        util.toggleClass(pre, 'codehighlight');
      });
    },
    switch3D: function() {
